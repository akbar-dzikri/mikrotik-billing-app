...
describe('GET /api/routers/[id]', () => {
  ...
  it('returns a router by id', async () => {
    mockDb.select.mockReturnValue(
      createMockQuery([buildRouter({ id: 'router-1', name: 'My Router' })]),
    );
...
describe('PUT /api/routers/[id]', () => {
  ...
  it('updates a router', async () => {
    mockDb.select.mockReturnValueOnce(
      createMockQuery([buildRouter({ id: 'router-1', name: 'Old Name' })]),
    );
    mockDb.select.mockReturnValueOnce(
      createMockQuery([buildRouter({ id: 'router-1', name: 'New Name' })]),
    );
    const res = await PUT(buildRequest('PUT', '/api/routers/router-1', { name: 'New Name' }), {
      params,
    });
    const body = await res.json();
    expect(res.status).toBe(200);
  ...
  it('re-encrypts password when password is provided', async () => {
    ...
    mockDb.select.mockReturnValueOnce(createMockQuery([buildRouter({ id: 'router-1' })]));
    const res = await PUT(
      buildRequest('PUT', '/api/routers/router-1', { password: 'new-secret' }),
      { params },
    );
    expect(res.status).toBe(200);
  ...
  it('returns 500 when session throws', async () => {
    mockGetSession.mockRejectedValue(new Error('Unauthorized'));
    const res = await PUT(buildRequest('PUT', '/api/routers/router-1', { name: 'Test' }), {
      params,
    });
    expect(res.status).toBe(500);
  });
