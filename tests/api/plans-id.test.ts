...
describe('GET /api/plans/[id]', () => {
  ...
  it('returns a plan by id', async () => {
    mockDb.select.mockReturnValue(createMockQuery([{ id: 'plan-1', name: 'Test Plan', type: 'hotspot', routerId: 'router-1', routerName: 'Test Router', sharedUsers: 1, validity: 30, price: '0', enabled: true, rateLimitDown: null, rateLimitUp: null, burstLimit: null, timeLimit: null, dataLimit: null, poolId: null, createdAt: new Date(), updatedAt: new Date() }]));
...
describe('PUT /api/plans/[id]', () => {
  ...
  it('updates a plan', async () => {
    mockDb.select.mockReturnValueOnce(createMockQuery([buildPlan({ id: 'plan-1', name: 'Old Plan' })]));
    mockDb.select.mockReturnValueOnce(createMockQuery([buildPlan({ id: 'plan-1', name: 'Updated Plan' })]));
    const handler = { updatePlan: vi.fn().mockResolvedValue(undefined) };
    mockGetDeviceHandler.mockReturnValue(handler);
    const res = await PUT(buildRequest('PUT', '/api/plans/plan-1', { name: 'Updated Plan' }), { params });
    const body = await res.json();
    expect(res.status).toBe(200);
